...
def classify_audio(audio_path):
    ...
    with torch.no_grad():
        ...
        sims_xin = [cos_sim(emb, g) for g in gallery_emb_xin]
        if np.mean(sims_xin) > threshold:
            return "XiaoXin"

    # Step 3: xiaoyuan vs others
    with torch.no_grad():
        emb = model_yuan(x).cpu().numpy().squeeze()
        sims_yuan = [cos_sim(emb, g) for g in gallery_emb_yuan]
        if np.mean(sims_yuan) > threshold:
            return "XiaoYuan"
...
if __name__ == "__main__":
    ...
    test_dir = Path("./model_simulation_validation/model_test/test_set")

    results = []  # 用于存储所有音频的识别结果

    for wav_path in test_dir.rglob("*.wav"):   # 支持递归地搜索所有子文件夹
        if "Xiao" in wav_path.name:
